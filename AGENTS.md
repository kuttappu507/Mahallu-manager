...
- Nav transitions: `MainShell` NavHost transitions are route-aware. Tab routes (`dashboard`/`families`/`members`/`finance`/`more`) crossfade quickly (`fadeIn(tween(150))`/`fadeOut(tween(110))`); pushed detail/form routes use a gentle scale+fade (`scaleIn 0.97`/`scaleOut 0.98` + fade) with NO translation — slide transitions caused restored tabs to stick at their translation offset (fixed in `e9a6b62`). The bottom bar itself is wrapped in `AnimatedVisibility` (slide + expand/shrink + fade) so it animates in/out instead of hard-cutting when navigating to a detail screen.
