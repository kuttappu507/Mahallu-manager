...
- Default-credential enforcement: `SeedData.isDefaultCredential(username, password)` flags the seeded `admin`/`secretary`/`treasurer` passwords; `LoginViewModel` sets `SessionManager.KEY_MUST_CHANGE_PASSWORD` and `AuthState.mustChangePassword`, and `MahalluNavGraph` routes to `change_password?forced=true` until changed. `ChangePasswordScreen` (feature-auth) requires the current password, min 6 chars; Settings → Account has a "Change Password" button (`settings` route passes `onChangePassword`).
