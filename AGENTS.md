...
- Logo: `core-ui/res/drawable/ic_logo_mosque.xml` (white mosque + coral door/windows, transparent bg) + `ic_logo_monochrome.xml` (all-white). Used by: launcher adaptive icons (foreground+monochrome), splash (`windowSplashScreenAnimatedIcon`), LoginScreen tile, and the `isInitializing` branded loading gate in `MahalluNavGraph`. Old `ic_launcher_foreground/monochrome` + `ic_splash` drawables removed (legacy PNG mipmaps unchanged for API<26).
