...
- PDF text color gotcha: Android `PdfDocument` renders `Paint.isFakeBoldText=true` with a synthetic "KeepDistance" font that FORCES the fill color to black — teal/gold/dark-bold text came out black in generated PDFs. Use `typeface = Typeface.DEFAULT_BOLD` (or `DEFAULT_BOLD`/`DEFAULT` per line) instead. Do NOT reintroduce `isFakeBoldText` in `feature-certificates`/`feature-reports` `PdfGenerator.kt`.
