...
- AGP 8.7.3, compileSdk/targetSdk 35, Gradle 8.9. Unused sensitive permissions were removed from the manifest (no more lint CAMERA error).
...
`AppCard` (shadow container), `AppButton`, `AppTextField` (+`PasswordTextField`), `ChipPill`, `SearchableSelectField`, `SimpleSelectField` (non-searchable dropdown, `options: List<Pair<String,String>>`), `ScreenPageHeader` (title + optional count pill + trailing icon button — used by Members/Families/Finance list screens), `ProfileHeroCard` (gradient hero with initials circle + chips — used by Member/Family detail), `TopAppBar`, `BottomNavigationBar`, `DashboardHeader`, `StatTile`, `SmallActionButton`, `BarChart`/`LineChart`/`ChartPoint`, `SectionHeader`/`DetailSectionTitle`/`InfoGridCard`, `AnimatedReveal`, `statusBarInsetDp()`.

## Shared Utils (`core-ui/.../util/`)
`Formatters` (below) + `PhoneActions.kt`: `dial(context, number)` / `whatsapp(context, number)` (shared by Family/Member detail screens).
...
- Nav transitions: `MainShell` renders the 5 tab screens via `when(activeTab)` OUTSIDE the NavHost (selected tab swapped via state, ViewModels stay warm/activity-scoped). The NavHost is ALWAYS composed (tab destinations are empty placeholders so `navController.graph` is always set — gating it behind a non-tab `currentRoute` meant the graph was never set at startup and the first tab click crashed with `IllegalStateException: You must call setGraph()`). `LaunchedEffect(currentRoute)` keeps `activeTab` synced when the back stack lands on a tab (system back). Tab-to-tab switches use `tabEnter()`/`tabExit()` = `EnterTransition.None`/`ExitTransition.None`; pushed detail/form routes use a gentle scale+fade (`fadeIn(160)` + `scaleIn 0.98`) with a fast `fadeOut(90)` exit — NO translation (slide transitions caused restored tabs to stick at their translation offset, fixed in `e9a6b62`). The bottom bar is wrapped in `AnimatedVisibility` (`expandVertically` from bottom + fade) so it grows/shrinks smoothly instead of hard-cutting when navigating to a detail screen. The auth `MahalluNavGraph` (login ↔ main ↔ change_password) uses explicit short fades (`180`/`120`) — navigation-compose's default is a 700ms crossfade which felt laggy.
...
- Receipt UX (IMPORTANT): Donation/subscription entry screens NEVER auto-navigate away after save (`saved && pdfPath==null` used to call `onDone()` silently, dropping the user with no receipt). On PDF failure they show an error + Retry + Done; Retry regenerates from the stored entity (no duplicate insert). Save-icon in entry TopAppBar is disabled once saved. Donation/subscription list rows have a trailing PDF quick-action icon that regenerates + opens the receipt directly (matching certificates tap-to-preview); row tap still opens the detail screen (View/Share). Entry ViewModels keep a `savedEntity` field + `retry()`; list ViewModels expose `generateReceipt(id, onResult)`.
...
- PDF text color gotcha: Android `PdfDocument` renders `Paint.isFakeBoldText=true` with a synthetic "KeepDistance" font that FORCES the fill color to black — teal/gold/dark-bold text came out black in generated PDFs. Use `typeface = Typeface.create(Typeface.SERIF, Typeface.BOLD)` (or `Typeface.SERIF` normal) instead. Do NOT reintroduce `isFakeBoldText` in `feature-certificates`/`feature-reports` `PdfGenerator.kt`.
- Design C preview match: certificate PDFs use the SERIF typeface (preview shows Georgia), bronze-gold `#7A5C16` for title/section/panel headers (`goldTextColor`, frame gold stays `#D9A441`), address `#55666F`, panel cards = centered uppercase headers + right-aligned values + light `#FBFDFC` bg + rounded corners. `PdfDocumentSpec` gained `goldTextColor` (default `#7A5C16`).
- Form screens (all add/edit/entry/login/settings) apply `Modifier.imePadding()` before `verticalScroll` so bottom fields stay visible above the keyboard (edge-to-edge disables `adjustResize`).
- Release prep: AGP 8.7.3 + compileSdk/targetSdk 35; unused manifest permissions removed (CAMERA, READ_MEDIA_IMAGES, storage, FOREGROUND_SERVICE*); `data_extraction_rules` cloud-backup now excludes `mahallu_db` (matches legacy `backup_rules`); Backup screen relabelled to "Local Backup" (Google Drive upload is a stub — `GoogleDriveManager.upload` returns null, do NOT advertise Drive); backup master key stored in EncryptedSharedPreferences (`mahallu_secure_prefs_enc`, migrated from legacy plain `mahallu_secure_prefs`).
- Default-credential enforcement: `SeedData.isDefaultCredential(username, password)` flags the seeded `admin`/`secretary`/`treasurer` passwords; `LoginViewModel` sets `SessionManager.KEY_MUST_CHANGE_PASSWORD` and `AuthState.mustChangePassword`, and `MahalluNavGraph` routes to `change_password?forced=true` until changed. `ChangePasswordScreen` (feature-auth) requires the current password, min 6 chars; Settings → Account has a "Change Password" button (`settings` route passes `onChangePassword`). The old "Continue as Guest" button (which auto-logged-in with hardcoded `secretary`/`secretary123`) was REMOVED — do not reintroduce hardcoded credentials in UI.
- No fake UI data: More-screen profile stats are live counts (Members/Families/Certificates via MemberRepository/FamilyRepository/CertificateRepository); finance balance trend badge computes a real month-over-month % (hidden when prev-month balance is 0). Collection entry prefills amount from the `default_subscription_amount` setting (NOT a hardcoded "500"). Announcement "Sent to N members" uses the real member count (`AnnouncementViewModel`).
